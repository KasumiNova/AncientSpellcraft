...
public class ItemWarlockOrb extends Item implements ISpellCastingItem, IWorkbenchItem, IWizardClassWeapon, IManaStoringItem {
	...
	public ItemWarlockOrb(Tier tier, Element element) {
		super();
	...
	private static EnumHand getOtherHandForSword(ItemStack stack, EntityLivingBase entity) {
		return entity.getHeldItemMainhand().getItem() instanceof ItemWarlockOrb ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND;
	}
	...
	public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
		...
		if (!oldStack.isEmpty() || !newStack.isEmpty() && oldStack.getItem() == newStack.getItem() && !slotChanged && oldStack.getItem() instanceof ItemWarlockOrb
				&& newStack.getItem() instanceof ItemWarlockOrb) { return false; }
	...
	public boolean canCast(ItemStack stack, Spell spell, EntityPlayer caster, EnumHand hand, int castingTick, SpellModifiers modifiers) {
		...
		if (castingTick == 0) {
			if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Pre(AncientSpellcraft.WARLOCK_ITEM, spell, caster, modifiers))) { return false; }
		} else {
			if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Tick(AncientSpellcraft.WARLOCK_ITEM, spell, caster, modifiers, castingTick))) { return false; }
		}
	...
	public boolean cast(ItemStack stack, Spell spell, EntityPlayer caster, EnumHand hand, int castingTick, SpellModifiers modifiers) {
		...
		if (spell.cast(world, caster, hand, castingTick, modifiers)) {
			...
			if (castingTick == 0) { MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Post(AncientSpellcraft.WARLOCK_ITEM, spell, caster, modifiers)); }
	...
	public void onPlayerStoppedUsing(ItemStack stack, World world, EntityLivingBase user, int timeLeft) {
		...
		if (user instanceof EntityPlayer) {
			...
			if (spell.isContinuous && spell.getTier().level <= this.tier.level && cost <= this.getMana(stack)) {
				...
				MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Finish(AncientSpellcraft.WARLOCK_ITEM, spell, player, modifiers, castingTick));
