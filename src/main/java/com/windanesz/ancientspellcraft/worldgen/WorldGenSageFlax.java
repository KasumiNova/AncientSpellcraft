...
public class WorldGenSageFlax implements IWorldGenerator {

	@Override
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider){

		if(Ints.contains(Wizardry.settings.flowerDimensions, world.provider.getDimension())){
			this.generatePlant(ASBlocks.sage_flax.getDefaultState(), world, random, 8 + chunkX * 16, 8 + chunkZ * 16, 1, 1);
		}
	}
