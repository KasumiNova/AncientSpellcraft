...
public class EntityChaosField extends EntityMagicConstruct {

	public EntityChaosField(World world) {
		super(world);
		this.height = 0.1f;
		this.width = 0.1f;
	}
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote) {
			...
			int amplifier = rand.nextInt(17);

			for (EntityLivingBase target : targets) {

				// All entities are slowed, even the caster (except those immune to frost effects)
				if (!target.isPotionActive(ASPotions.chaos)) {
					target.addPotionEffect(new PotionEffect(ASPotions.chaos, 140, amplifier));
				}

				Map<Potion, PotionEffect> activeEffects = new HashMap<>(target.getActivePotionMap());
				for (Potion potion : activeEffects.keySet()) {
					//if (potion instanceof PotionMagicEffect && !(potion instanceof Curse) && potion != ASPotions.chaos) {
			...
			for (int i = 0; i < 20; i++) {
				...
				ParticleBuilder.create(WarlockElementalSpellEffects.getElementalParticle(Element.MAGIC))
						.pos(posX + offsetX, posY + radius * world.rand.nextFloat(), posZ + offsetZ)
						.vel(0, 0.05 + (world.rand.nextFloat() * 0.1), 0)
						.spin(0.3, 0.1)
						.clr(colours[0]).fade(colours[1]).scale(2).time(40).shaded(false).spawn(world);
			}
